...
Bool-𝟚-isomorphism = record { bijection = f ; bijectivity = record {inverse = g ; η = gf ; ε = fg } }
 where
  f : Bool → 𝟚
  f false = 𝟎
  f true  = 𝟏
...
well-ordering-principle-suc P d n p (inl p0) _  = 0 , p0 , λ n pn → ⋆
well-ordering-principle-suc P d n p (inr neg-p0) (m , pm , is-min-m) = suc m , pm , is-min-suc-m
  where
    is-min-suc-m : is-lower-bound P (suc m)
    is-min-suc-m zero pn = neg-p0 pn
    is-min-suc-m (suc n) pn = is-min-m n pn
```
...
```agda
well-ordering-principle : (P : ℕ → Type) → (d : is-decidable-predicate P) → (n : ℕ) → P n → minimal-element P
well-ordering-principle P d 0 p = 0 , p , λ n pn → ⋆ 
...
is-zero-well-ordering-principle P d (suc m) pm = is-zero-well-ordering-principle-suc P d m pm (d 0) (well-ordering-principle (λ m → P (suc m)) (λ m → d (suc m)) m pm)
```
